...
class Orchestrator:
    ...
    def _handle_meeting(self, state: MCPState) -> MCPState:
        """Full pipeline: calendar + web research (always) + ToT brief."""
        ...
        # Step 2: build research queries from calendar + user input
        # Always run web search — never rely on LLM training data for briefs
        queries = _build_all_research_queries(state.calendar_data, state.user_input)
        logger.info(f"Running {len(queries)} research queries")

        all_research = []
        for i, query in enumerate(queries, 1):
            logger.info(f"Research query {i}/{len(queries)}: {query[:80]}")
            temp_state = MCPState(user_input=query)
            temp_state = self.research_agent.run(temp_state, query)
            if temp_state.research_output and len(temp_state.research_output) > 100:
                all_research.append(f"### Query {i}: {query}\n{temp_state.research_output}")

        # Combine all research into state
        state.research_output = "\n\n---\n\n".join(all_research)
        state.sources_valid   = len(state.research_output) > 200
        logger.info(f"Total research collected: {len(state.research_output)} chars")

        # Step 3: generate brief using all research
...
def _build_all_research_queries(calendar_data: str, user_input: str) -> list:
    """
    Build a list of targeted web search queries from the meeting
    title, agenda items, and user input.
    Always returns at least 3 queries so the Research Agent
    ...
    # Use meeting title or fall back to user input as base topic
    base = title if title else user_input

    queries = []

    # Query 1 — overview and types
    queries.append(f"{base} overview types explained")

    # Query 2 — implementation and best practices
    queries.append(f"{base} implementation best practices 2024")

    # Query 3 — limitations and challenges
    queries.append(f"{base} limitations challenges problems")

    # Query 4 — one per key agenda item (up to 2 more)
    for item in agenda[:2]:
        if len(item) > 10:
            queries.append(f"{base} {item}")

    # Query 5 — if user added extra context beyond generic phrases
    generic = {"prepare a brief", "meeting brief", "prep for my meeting", "brief for"}
    if user_input and user_input.lower().strip() not in generic:
